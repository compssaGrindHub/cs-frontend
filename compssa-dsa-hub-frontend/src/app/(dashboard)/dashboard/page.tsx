...
export default function DashboardPage() {
  ...
  const isLoading = userLoading || statsLoading || rankLoading || dailyLoading || contestsLoading || submissionsLoading || progressLoading;
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-sm text-gray-400">
                        {daily.dailyDate ? formatDistanceToNow(new Date(daily.dailyDate), { addSuffix: true }) : 'Today'}
                      </p>
                    </div>
                  </div>
                  <div>
                    <h3 className="text-white font-semibold mb-1">{daily.title}</h3>
                    <div className="flex items-center gap-2 mb-3 flex-wrap">
                      <Badge variant="outline" className={`border-${
                        daily.difficulty === 'HARD' ? 'red' : 
                        daily.difficulty === 'MEDIUM' ? 'yellow' : 'green'
                      }-500/50 text-${
                        daily.difficulty === 'HARD' ? 'red' : 
                        daily.difficulty === 'MEDIUM' ? 'yellow' : 'green'
                      }-500 text-xs`}>
                      ...
                      {daily.topics.slice(0, 2).map((topic) => (
                        <Badge key={topic} variant="outline" className="border-blue-500/50 text-blue-500 text-xs">
                          {topic}
                        </Badge>
                      ))}
                    </div>
              ...
              <div className="space-y-4">
                {recentSubmissions.length > 0 ? (
                  recentSubmissions.slice(0, 5).map((submission) => (
                    <div key={submission.id} className="flex gap-3 pb-4 border-b border-white/5 last:border-0 last:pb-0">
              ...
              <div className="space-y-3">
                {upcomingContests.length > 0 ? (
                  upcomingContests.map((contest) => (
                    <div key={contest.id} className="p-4 rounded-lg bg-white/5 border border-white/5">
