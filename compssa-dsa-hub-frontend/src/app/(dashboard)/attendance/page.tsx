        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
          <div>
            ...
            <p className="text-xs md:text-sm text-muted-foreground">
              Your attendance overview across sessions.
            </p>
          </div>
          <Button
            variant="outline"
            className="gap-2 w-full sm:w-auto text-xs md:text-sm h-8 md:h-10"
          >
          ...
          <CardContent className="p-4 md:p-6 space-y-3 md:space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 md:gap-3">
              <div>
                ...
                <p className="text-xs md:text-sm text-muted-foreground">
                  Recent sessions (sample data for now).
                </p>
              </div>
              <div className="flex items-center gap-1.5 md:gap-2 text-[10px] md:text-xs text-muted-foreground">
                ...
                {[0, 1, 2, 3, 4].map((level) => (
                  <span
                    key={level}
                    className={`w-2.5 h-2.5 md:w-3 md:h-3 rounded-sm border ${levelColors[level as keyof typeof levelColors]}`}
                  />
                ))}
                <span>More</span>
              </div>
            ...
            <div className="overflow-x-auto -mx-4 md:mx-0 px-4 md:px-0">
              <div className="grid grid-cols-12 gap-0.5 md:gap-1 min-w-[400px]">
                {heatmapData.map((entry) => (
                  <div key={entry.date} className="space-y-1">
                    <div
                      title={`${entry.date}: ${entry.level > 0 ? "Present" : "No attendance"}`}
                      className={`w-full h-4 md:h-6 rounded-sm border ${levelColors[entry.level as keyof typeof levelColors]}`}
                    />
                  </div>
                ))}
              </div>
          ...
          <CardContent className="p-4 md:p-6 space-y-3 md:space-y-4">
            ...
            {recentAttendance.length === 0 ? (
              <EmptyState
                ...
                description="Attend sessions to see your progress here."
              />
            ) : (
              <div className="space-y-2 md:space-y-3">
                {recentAttendance.map((entry) => (
                  <div
                    key={entry.id}
                    className="flex items-center justify-between gap-2 border-b border-border last:border-0 pb-2 md:pb-3 last:pb-0"
                  >
                    <div className="flex-1 min-w-0">
                      <div className="text-xs md:text-sm font-medium text-foreground truncate">
                        {entry.session?.name || "Session"}
                      </div>
                      <div className="text-[10px] md:text-xs text-muted-foreground">
                        {entry.session?.date
                          ? format(new Date(entry.session.date), "MMM dd, yyyy")
                          : "Unknown date"}
