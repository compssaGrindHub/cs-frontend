...
import {
  ...
  getProblemSolveRate,
  getAdminContestParticipationRate,
  getEngagementChart,
...
export default function AdminAnalyticsPage() {
  ...
  const { data: topicDistribution, isLoading: topicDistributionLoading } =
    useQuery({
      queryKey: ["admin", "topicDistribution"],
      queryFn: getTopicDistribution,
    });

  const { data: problemSolveRate, isLoading: problemSolveRateLoading } =
    useQuery({
  ...
  const {
    data: contestParticipationRate,
    isLoading: contestParticipationRateLoading,
  } = useQuery({
    queryKey: ["admin", "contestParticipationRate", timeRange],
    queryFn: () => getAdminContestParticipationRate(getDateRange()),
  });
  ...
  const { data: attendanceAnalytics, isLoading: attendanceAnalyticsLoading } =
    useQuery({
  ...
  const isLoading =
    ...
    problemSolveRateLoading ||
    contestParticipationRateLoading ||
    engagementChartLoading ||
    attendanceAnalyticsLoading;

  const totalHoursSpent = usageTimeStats?.overall
    ? Math.floor(usageTimeStats.overall / 60)
    : 0;
  const totalMinutesRemainder = usageTimeStats?.overall
    ? usageTimeStats.overall % 60
    : 0;
  const avgHoursSpent = usageTimeStats?.average
    ? Math.floor(usageTimeStats.average / 60)
    : 0;
  const avgMinutesRemainder = usageTimeStats?.average
    ? Math.floor(usageTimeStats.average % 60)
    : 0;
  ...
  const stats = [
    ...
    },
    {
      ...
      value: overview?.submissions?.toLocaleString() || "0",
      change: "+18%",
      icon: Code2,
    ...
    },
    {
      ...
      value: `${totalHoursSpent}h ${totalMinutesRemainder}m`,
      change: `${avgHoursSpent}h ${avgMinutesRemainder}m avg`,
      icon: Calendar,
  ...
  const topicData =
    topicDistribution?.slice(0, 10).map((item, index) => {
      const colors = [
        "#3b82f6",
        "#10b981",
        "#f59e0b",
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-blue-500">
                  {attendanceAnalytics?.items &&
                  attendanceAnalytics.items.length > 0
                    ? Math.round(
                        (attendanceAnalytics.items.reduce(
                          (sum, item) => sum + item.percentage,
                          0
                        ) /
                          attendanceAnalytics.items.length) *
                          100
                      ) / 100
                    : 0}
                  %
        ...
        <Card className="bg-gradient-to-br from-green-500/10 to-green-500/5 border-green-500/20 shadow-sm">
          <CardContent className="p-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm text-muted-foreground">
                  Avg Problem Solve Rate
                </p>
                <p className="text-2xl font-bold text-green-500">
                  {problemSolveRate?.solveRate || 0}%
                </p>
                <p className="text-xs text-muted-foreground mt-1">
                  Platform-wide
                </p>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-purple-500">
                  {contestParticipationRate?.participationRate || 0}%
                </p>
                <p className="text-xs text-muted-foreground mt-1">
                  Active members
                </p>
